const message = '5~CCCAGG~BTC~USD~4~6526.9~1534732009~0.00116~7.5480272~49251009~4279.785801874884~27936734.20621795~39748.68586736848~256420172.90350387~6502.18~6537.54~6499.66~6387.08~6572.25~6334.33~Coinbase~7ffe9';
const message2 = '5~CCCAGG~BTC~USD~2~6527.04~4280.408599984883~27940787.374352403~39749.29856547847~256424160.35194832~f01';
let CCC = {};
...
function dataUnpack (currentPrice, message) {
    let data = CCC.CURRENT.unpack(message);
    console.log(data);
    ...
    let pair = from + to;
    console.log(from,to,fsym, tsym, pair);
    ...
    return [ currentPrice[pair], from, tsym, fsym ];
};
function decorateWithFullVolume (currentPrice, message) {
    let volData = CCC.FULLVOLUME.unpack(message);
    console.log('vol', volData);
    let from = volData['SYMBOL'];
    let to = 'USD';
    let fsym = CCC.STATIC.CURRENCY.getSymbol(from);
    let tsym = CCC.STATIC.CURRENCY.getSymbol(to);
    let pair = from + to;
...
function decodeMessage(message)  {
    ...
    if (messageType == CCC.STATIC.TYPE.CURRENTAGG) {
        dataUnpack(currentPrice, message);
    }
    else if (messageType == CCC.STATIC.TYPE.FULLVOLUME) {
        decorateWithFullVolume(currentPrice, message);
    }
}
